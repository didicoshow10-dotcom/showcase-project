...
interface EditableImageProps {
  ...
  height: number;
  className?: string;
  /** Slide-space scale factor, so drag deltas track the cursor 1:1. */
  scale: number;
  /** Image is decorative background art rather than a logo. */
  cover?: boolean;
}
...
/**
 * Logo / image slot that can be replaced, moved and resized while the deck is
 * in edit mode. Position and size are stored in 1920x1080 slide coordinates.
 */
  ...
  const beginDrag = useCallback(
    (event: React.PointerEvent, kind: DragKind) => {
      ...
      const onMove = (moveEvent: PointerEvent) => {
        ...
        // Corner resize, proportional to the original aspect ratio.
        ...
        const nextX =
          kind === "nw" || kind === "sw"
            ? origin.x + (origin.width - nextWidth)
            : origin.x;
        const nextY =
          kind === "nw" || kind === "ne"
            ? origin.y + (origin.height - nextHeight)
            : origin.y;
