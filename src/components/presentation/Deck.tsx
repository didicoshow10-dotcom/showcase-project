...
/** Stacked layout for `?print` — one 1920x1080 page per slide. */
function PrintDeck() {
  return (
...
export function Deck() {
  ...
  const goTo = useCallback((next: number) => {
    setIndex(Math.min(Math.max(next, 0), TOTAL_SLIDES - 1));
  }, []);

  const goNext = useCallback(() => setIndex((i) => Math.min(i + 1, TOTAL_SLIDES - 1)), []);
  const goPrev = useCallback(() => setIndex((i) => Math.max(i - 1, 0)), []);

  /* Keep the URL and tab title in sync without polluting history. */
  useEffect(() => {
  ...
  return (
    <div className="flex h-[100dvh] flex-col bg-ink text-primary-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          {editMode ? (
            <button
              type="button"
              onClick={restore}
              className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-xs font-semibold text-white/85 transition hover:bg-white/20"
            >
      ...
      >
        <ScaledSlide className="size-full">
          {(scale) => (
            <div key={hydrated ? "ready" : "ssr"} className="size-full">
              {renderSlide(index, scale)}
            </div>
          )}
        </ScaledSlide>
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-4 flex justify-center">
          <div className="pointer-events-auto flex items-center gap-3 rounded-full bg-black/55 px-3 py-2 backdrop-blur">
            <button
              ...
              onClick={goPrev}
              disabled={index === 0}
              className="rounded-full p-2 text-white/85 transition hover:bg-white/15 disabled:opacity-30"
              aria-label="Slide anterior"
            >
              <ChevronLeft className="size-5" aria-hidden="true" />
            </button>
            <span className="min-w-[62px] text-center text-xs font-semibold tabular-nums text-white/85">
      ...
      <div className="h-1.5 w-full shrink-0 bg-white/10" role="presentation">
        <div
      ...
      {gridOpen ? (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-ink/97 p-5 sm:p-9">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-base font-semibold">Todos os slides</h2>
            <button
              type="button"
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {SLIDE_TITLES.map((title, slideIndex) => (
              <button
                key={title}
                type="button"
                onClick={() => {
              ...
              >
                <ScaledSlide className="aspect-video w-full">
                  {(scale) => renderSlide(slideIndex, scale)}
                </ScaledSlide>
                <p className="truncate px-3 py-2 text-xs text-white/70">
                  {String(slideIndex + 1).padStart(2, "0")} · {title}
                </p>
              </button>
            ))}
          </div>
