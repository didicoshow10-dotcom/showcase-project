...
export function CoverSlide({ index, scale, content, isPrimary = false }: CoverSlideProps) {
  ...
  return <CoverFrame index={index} scale={scale}>
    ...
    </> : isSynthesis ? <div className="w-full max-w-[1120px]">
      <div className="inline-flex rounded-full border-2 border-primary-foreground/45 px-6 py-2">
        <EditableText id={`cover-badge-${index}`} value={content.badge} className="slide-kicker text-primary-foreground" singleLine />
      </div>
      <EditableText id={`cover-title-${index}`} as="h2" value="Da leitura à tração" className="slide-title-lg mt-5 block max-w-[900px] text-primary-foreground" />
      <EditableText id={`cover-subtitle-${index}`} value="Uma sequência simples para transformar diagnóstico em resultado" className="slide-subtitle mt-2 block max-w-[980px] text-lilac-deep" />
      <div className="mt-7 grid grid-cols-2 gap-5">
        {lines.map((line,i)=>{
          const parts = line.split("|");
          const label = parts[0]?.trim() ?? "";
          const body = parts.slice(1).join("|").trim();
          return <article key={i} className="flex min-h-[112px] flex-col justify-center rounded-[20px] border border-primary-foreground/20 bg-black/10 px-6 py-5">
            <EditableText id={`cover-line-label-${index}-${i}`} as="p" value={label} className="text-[17px] font-bold uppercase tracking-[0.04em] text-lilac-deep" singleLine />
            <EditableText id={`cover-line-body-${index}-${i}`} as="p" value={body} className="mt-2 block text-[20px] leading-[1.28] text-primary-foreground" />
          </article>;
        })}
      </div>
    </div> : <>
